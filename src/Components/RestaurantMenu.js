    ...
    return (!restaurant) ? <Shimmer /> : (
        <div className="flex">
           ...
           <div className="m-10">
            ...
            <ul>{(restaurant?.cards[2]?.groupedCard?.cardGroupMap?.REGULAR?.cards[2]?.card?.card?.itemCards).map((itemCard) => 
            <li key={itemCard?.card?.info?.id}>{itemCard?.card?.info?.name} - <button className="bg-amber-300 font-semibold p-1 m-3 w-44 border-4 rounded-lg border-amber-100" onClick={() => handleCart(itemCard?.card?.info?.name)}>Add</button></li>)}</ul>
           </div>
        </div>
    )
}
